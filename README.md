...
To address these challenges, this paper proposes **InfTS-LLM**, a zero-shot method composed of two components: (a) a Representative Detector efficiently extracts representative subsequences with temporal patterns using a linear-time Cascade Retraced Detection algorithm and (b) a Feedback Instructor constructs pattern-guided chains of thought for deep reasoning with LLMs and generates feedback scores to maintain the Memory Pool with an effective eviction mechanism. 

...
if __name__ == '__main__':
    ...
    for dataset_path in DATASET_PATHS.values():
        ...
        # 处理事件预测数据集
        elif "PREDICTION" in TASK and any(dataset_name in DATASET_PREDICTION for dataset_name, path in DATASET_PATHS.items() if path == dataset_path):
    ...
    if "REASONING" in TASK:
       # 创建ChatTS目录并合并stream_summary.csv文件
       #  print("Generating ChatTS Merged streaming samples:")

        # 创建ChatTS目录
        qats4_dir = os.path.join(project_root, "datasets", "ChatTS")
        os.makedirs(qats4_dir, exist_ok=True)

        # 创建stream-ChatTS目录
        stream_qats4_dir = os.path.join(qats4_dir, "stream-ChatTS")
        ...
        if merged_df is not None:
            ...
            print(f"- Processed ChatTS: {len(merged_df)} series \n")
        else:
